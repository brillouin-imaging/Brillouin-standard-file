...
- **‘/’** (root group) must have the following attributes:
  ...
  - **‘Subtype’ [string]** (optional): identifier of the specific subtype of the current file. The idea is that specific implementations of Brillouin might standardize additional groups, arrays or attributes (features). Subtypes (and corresponding features) must be defined in ['brim_file_subtypes'](brim_file_subtypes.md); when present, it is reccomended to include also ‘Subtype_features’
