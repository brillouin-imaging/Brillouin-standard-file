...
- **‘/Data_{n}/Scanning/Cartesian\_visualization’ [3D int]** (optional) this array is only relevant when ‘Sparse’ is true; it contains an index which associates the position in the 3D grid to the first dimension in ‘PSD’ (thus obviously the index must be smaller than *N\_points*). The order of dimensions is ZYX and the 3 dimensions must always be present, where the unused dimensions can be set to 1. If the scanning is done in non-cartesian coordinates -1 can be included to fill the "empty" pixels
  ...
  N.B. in principle, the 3D grid could be reconstructed from the array '/Scanning/Spatial\_map' (if present), but, when 'Sparse' is true, it is good to have '/Scanning/Cartesian\_visualization' always defined to avoid computing the assignments of spectra to 3D coordinates every time and also to allow for different ways for reconstructing the image (in case it is useful)
