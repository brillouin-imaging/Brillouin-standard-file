...
Individual spectra are stored in the '/data_{n}/PSD' array, where the first three dimensions represent the spatial coordinates (in the order z, y, x), while the last dimension contains the spectral information. The format also allows for the storage of sparse data where the spectra are stored as a flattened array and the mapping of these spectra to a 3D image is done through the information in the '/data_{n}/Scanning/' group.
...
## General features:
- The terminology used in this document follows that of [Zarr](https://zarr-specs.readthedocs.io/en/latest/v3/core/index.html#concepts-and-terminology), even though these specifications only define the structure of the file and are thus not restricted to Zarr
...
- If supported by the underlying storage, links to internal groups/arrays can be used instead of duplicating the data, if needed
...
|   ---- ---- ---- /Shift\_S\_{p} [float] (optional)

|   ---- ---- ---- /Width\_AS\_{p} [float]

|   ---- ---- ---- /Width\_S\_{p} [float] (optional)

|   ---- ---- ---- /Amplitude\_AS\_{p} [float]

|   ---- ---- ---- /Amplitude\_S\_{p} [float] (optional)

|   ---- ---- ---- /Offset\_AS\_{p} [float]

|   ---- ---- ---- /Offset\_S\_{p} [float] (optional)
...
- **‘/Data_{n}/PSD’ [float]**: the last dimension contains the spectral information, while the first the spatial information. If ‘Sparse’ is true, the spatial information is 1D, i.e. contains a list of spatial positions in the sample (*N\_points*); if ‘Sparse’ is false the first 3 dimensions are z, y and x (with Nz\*Ny\*Nx = N\_points). Optionally (both in the sparse and not sparse case) ‘PSD’ can have more dimensions, accounting for the case when for each voxel in the sample multiple spectra are acquired (e.g. angle resolved measurements); the new dimensions must be inserted in-between the spectral and the spatial dimensions (i.e. the spatial and spectral dimensions must always be the first and the last, to make the broadcast of the ‘Frequency’ array easier) 
...
- **‘/Data_{n}/Raw\_data’** (optional group): this group should contain the raw data that are used to generate the spectra. The actual structure of the data is left undefined to allow full flexibility (different techniques/instruments might have very different raw data), but it might be better defined for a specific ‘Subtype’.
- **‘/Data_{n}/Scanning/Spatial\_map’** (optional): group containing the arrays ‘x’, ‘y’ and ‘z’ (of type float and with the same shape as the spatial coordinates in ‘PSD’), which refer to the coordinates on the sample at which the spectrum was acquired; if any of the ‘x’, ‘y’ and ‘z’ arrays is omitted it is considered filled with zeros. The units must be attached to the ‘Spatial\_map’ and not to the individual ‘x’, ‘y’, ‘z’ arrays (i.e. the units are the same for all 3 arrays). If ‘Cartesian\_visualisation’ is defined, ‘Spatial\_map’ can be omitted
- **‘/Data_{n}/Scanning/Cartesian\_visualisation’ [3D int]** (optional) this array is only relevant when ‘Sparse’ is true; it contains an index which associates the position in the 3D grid to the first dimension in ‘PSD’ (thus obviously the index must be smaller than *N\_points*). The order of dimensions is ZYX and the 3 dimensions must always be present, where the unused dimensions can be set to 1. If the scanning is done in non-cartesian coordinates -1 can be included to fill the "empty" pixels
...
- **‘/Data_{n}/Analysis\_{m}’** (group) contains the results of the analysis on the spectral data; the index 'm' allows for the case of multiple pipelines being performed on the same data (in that case a group for each of them must be created).
  ...
  Additionally, it contains the following arrays. All arrays must have the same shape as the spatial dimensions in 'PSD'. They can have more dimensions, in order to match the dimensions in the PSD array. They can optionally have both the results for anti-Stokes and Stokes, in case both are present (the recommendation is to use the average between the two, when displaying the image).
